...
public class NearHaltAdapter extends ArrayAdapter<String> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        bus = "";
        troll="";
        Cursor busCursor = database.rawQuery("select distinct Routes.transport,Routes.TYPE from Coordinates,Routes where Coordinates.NAME = '"+nearHaltList.get(position)+"' and Routes.STOPS like '%"+IdList.get(position)+"%'",null);
        while (busCursor.moveToNext()){
                switch (busCursor.getString(busCursor.getColumnIndex("TYPE"))){
                    case "A": if (bus.length()<35){bus+= busCursor.getString(busCursor.getColumnIndex("transport")) +" ";} break;
                    case "Т": if (troll.length()<35){troll+= busCursor.getString(busCursor.getColumnIndex("transport")) +" ";} break;
                }
        }
        busCursor.close();
        if(bus.length()>=35){
